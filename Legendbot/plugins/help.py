...
@legend.legend_cmd(
    ...
    info={
        "header": "To get guide for catLegendbot.",
...
async def _(event):
    ...
    await eor(
        event, outstr, aslink=True, linktext="Total Commands of Legenduserbot are :"
    )
...
async def _(event):
    ...
    if found := [i for i in sorted(list(CMD_INFO)) if cmd in i]:
        ...
        out = f"I can't find any such command `{cmd}` in LegendUserbot"
    await eor(event, out)
