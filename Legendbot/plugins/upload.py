...
async def legendlst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        legendpath = os.path.join(path, file)
        if os.path.isfile(legendpath):
            legendsort.append(file)
    for file in contents:
        legendpath = os.path.join(path, file)
        if os.path.isdir(legendpath):
            legendsort.append(file)
    return legendsort
...
async def upload(path, event, udir_event, legendflag=None):  # sourcery no-metrics
    legendflag = legendflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            legendpath = os.path.join(path, file)
            await upload(Path(legendpath), event, udir_event)
...
async def uploadir(event):
    ...
    if os.path.isdir(path):
        ...
        UPLOAD_.uploaded = 0
        await upload(path, event, udir_event, legendflag=flag)
        end = datetime.now()
