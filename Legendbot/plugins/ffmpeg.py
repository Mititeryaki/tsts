...
FF_MPEG_DOWN_LOAD_MEDIA_PATH = os.path.join(
    Config.TMP_DOWNLOAD_DIRECTORY, "catLegendbot.media.ffmpeg"
)
...
async def ffmpeg_compress(event):  # sourcery skip: low-code-quality
    ...
    if not crf:
        crf = "23"
    dlpath = os.path.join(Config.TMP_DOWNLOAD_DIRECTORY, "lol.media.ffmpeg")
    ...
    if cmd == "f":
        try:
            c_time = time.time()
            legendt = await event.client.send_file(
                event.chat_id,
                compress,
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        if mpath and os.path.exists(mpath):
            ...
            await _legendutils.runcmd(f"cp -r {mpath} {FF_MPEG_DOWN_LOAD_MEDIA_PATH}")
        ...
        if reply_message:
            ...
            if (
                ...
                return await eod(event, "`Only Video/Audio files are supported`", 5)
            legendevent = await eor(event, "`Saving the file...`")
            try:
                c_time = time.time()
