...
@legend.legend_cmd(
    ...
    info={
        ...
        "examples": "{tr}chatinfo @LegendBot_OP",
    },
)
async def info(event):
    ...
    chat = await get_chatinfo(event, match.strip(), legendevent)
    if not chat:
        return
...
@legend.legend_cmd(
    ...
    command=("grpstats", menu_category),
    info={
        ...
        "examples": [
            "{tr}grpstats @LegendBot_OP",
            "{tr}grpstats -q2000 @LegendBot_OP",
            "{tr}grpstats -l20 @LegendBot_OP",
            "{tr}grpstats -s @LegendBot_XDS",
            "{tr}grpstats -s -l20 -q2000 @LegendBot_XDS",
        ],
    },
)
async def grp_stat(event):  # sourcery skip: low-code-quality
    ...
    try:
        ...
        flag = None
    temp = {}
    chatinfo = await get_chatinfo(event, match.strip(), legendevent)
    if not chatinfo:
        return
    ...
    await legendevent.edit(string, parse_mode="html")
