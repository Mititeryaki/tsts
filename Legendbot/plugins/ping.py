import asyncio
import random
import time
from datetime import datetime

import requests
from telethon.errors.rpcerrorlist import (
    MediaEmptyError,
    WebpageCurlFailedError,
    WebpageMediaEmptyError,
)

from ..core.managers import eor
from ..helpers.functions import get_readable_time
from ..sql_helper.globals import gvarstatus
from . import StartTime, legend, mention, reply_id

menu_category = "tools"


temp_ = "__**☞ Pong**__"
temp = "__**☞ Pong**__\n➥ `{ping}` **ms**\n➥ __**Bot of **__{mention}"


@legend.legend_cmd(
    pattern="ping( -a|$)",
    command=("ping", menu_category),
    info={
        "header": "check how long it takes to ping your userbot",
        "flags": {"-a": "average ping"},
        "usage": ["{tr}ping", "{tr}ping -a"],
    },
)
async def ping(event):
    "To check ping"
    flag = event.pattern_match.group(1)
    reply_to_id = await reply_id(event)
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    if flag == " -a":
        legendevent = await eor(event, "`!....`")
        await asyncio.sleep(0.3)
        await eor(legendevent, "`..!..`")
        await asyncio.sleep(0.3)
        await eor(legendevent, "`....!`")
        end = datetime.now()
        tms = (end - start).microseconds / 1000
        ms = round((tms - 0.6) / 3, 3)
        await eor(legendevent, f"Average Ping!\n`{ms} ms`")
    else:
        legendevent = await eor(event, temp_)
        end = datetime.now()
        ms = (end - start).microseconds / 1000
        ANIME = None
        ping_temp = gvarstatus("PING_TEMPLATE") or temp
        PING_PIC = gvarstatus("PING_PIC")
        if "ANIME" in ping_temp:
            data = requests.get("https://animechan.vercel.app/api/random").json()
            ANIME = f"**“{data['quote']}” - {data['character']} ({data['anime']})**"
        caption = ping_temp.format(
            ANIME=ANIME,
            mention=mention,
            uptime=uptime,
            ping=ms,
        )
        if PING_PIC:
            LEGEND = list(PING_PIC.split())
            PIC = random.choice(LEGEND)
            try:
                await event.client.send_file(
                    event.chat_id, PIC, caption=caption, reply_to=reply_to_id
                )
                await legendevent.delete()
            except (WebpageMediaEmptyError, MediaEmptyError, WebpageCurlFailedError):
                return await eor(
                    legendevent,
                    f"**Media Value Error!!**\n__Change the link by __`.setdb`\n\n**__Can't get media from this link :-**__ `{PIC}`",
                )
        else:
            await eor(
                legendevent,
                caption,
            )
