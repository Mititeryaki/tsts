...
async def legendgban(event):  # sourcery no-metrics
    ...
    if gban_sql.is_gbanned(user.id):
        ...
        gban_sql.legendgban(user.id, reason)
    krish = await admin_groups(event.client)
    count = 0
    krishna = len(krish)
    if krishna == 0:
        return await eod(cate, "`you are not admin of atleast one group` ")
    await cate.edit(
        f"`initiating gban of `[{user.first_name}](tg://user?id={user.id}) `in {len(krish)} groups`"
    )
    for i in range(krishna):
        try:
            await event.client(EditBannedRequest(krish[i], user.id, BANNED_RIGHTS))
            await asyncio.sleep(0.5)
            count += 1
        except BadRequestError:
            achat = await event.client.get_entity(krish[i])
            await event.client.send_message(
                BOTLOG_CHATID,
                f"`You don't have required permission in :`\n**Chat :** {get_display_name(achat)}(`{achat.id}`)\n`For banning here`",
            )
    end = datetime.now()
    legendtaken = (end - start).seconds
    if reason:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {legendtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {legendtaken} seconds`!!"
    ...
    if BOTLOG and count != 0:
        ...
        if reason:
            await event.client.send_message(
                ...
                \n**Time taken : **`{legendtaken} seconds`",
            )
        else:
            await event.client.send_message(
                BOTLOG_CHATID,
...
async def legendgban(event):
    ...
    await cate.edit(
        f"initiating ungban of [{user.first_name}](tg://user?id={user.id}) in `{len(krish)}` groups"
    )
    for i in range(krishna):
        try:
            await event.client(EditBannedRequest(krish[i], user.id, UNBAN_RIGHTS))
            await asyncio.sleep(0.5)
            count += 1
        except BadRequestError:
            achat = await event.client.get_entity(krish[i])
    ...
    if reason:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}`) was ungbanned in {count} groups in {legendtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was ungbanned in {count} groups in {legendtaken} seconds`!!"
...
async def legendgkick(event):  # sourcery no-metrics
    ...
    if krishna == 0:
        return await eod(cate, "`you are not admin of atleast one group` ")
    await cate.edit(
        f"`initiating gkick of the `[user](tg://user?id={user.id}) `in {len(krish)} groups`"
    )
    for i in range(krishna):
        try:
            await event.client.kick_participant(krish[i], user.id)
            await asyncio.sleep(0.5)
            count += 1
        except BadRequestError:
            achat = await event.client.get_entity(krish[i])
    ...
    if reason:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {legendtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {legendtaken} seconds`!!"
