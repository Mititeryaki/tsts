...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    if heroku_app is None:
        await event.edit(
            f"{txt}\n" "`Invalid Heroku credentials for deploying Legendbot dyno.`"
        )
        return repo.__del__()
    LEGEND = await event.edit(
...
async def upstream(event):
    ...
    try:
        ...
        if conf is None:
            return await event.edit(
                f"`Unfortunately, the directory {error} does not seem to be a git repository.\nBut we can fix that by force updating the Legendbot using .update now.`"
            )
        repo = Repo.init()
    ...
    if force_update:
        await event.edit(
            "`Force-Syncing to latest stable Legendbot code, please wait...`"
        )
    if conf == "now":
        await event.edit("`Updating Legendbot, please wait....`")
...
async def upstream(event):
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("`Deploying Legendbot, please wait....`")
...
async def variable(var):
    ...
    heroku_var["UPSTREAM_REPO"] = "https://github.com/LEGEND-AI/LEGENDUSERBOT"
