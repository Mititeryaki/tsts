...
# Legenduserbot Google Drive managers  ported from Projectbish and added extra things by @mrconfused
...
async def upload(gdrive, service, file_path, file_name, mimeType, dir_id=None):
    ...
    body = {
        "name": file_name,
        "description": "Uploaded from Telegram using LegendLegendbot.",
        "mimeType": mimeType,
        "parents": dir_id,
    }
...
async def generate_credentials(gdrive):
    ...
    if helper.get_credentials(str(hmm)) is not None:
        await eod(gdrive, "`You already authorized token...`")
        return
    ...
    async with gdrive.client.conversation(BOTLOG_CHATID) as conv:
        ...
        if getcode := re.search("(.*)(4\/.*)(&.*)", code):
            ...
            await eor(gdrive, "Given link is wrong. recheck your link")
            return
        try:
...
async def set_upload_folder(gdrive):
    ...
    await eor(gdrive, "`Sending information...`")
    if G_DRIVE_FOLDER_ID is not None:
...
async def set_upload_folder(gdrive):
    ...
    if not inp:
        return await eor(gdrive, ">`.gset <folderURL/folderID>`")
    try:
...
async def g_download(event):
    ...
    file_name, legendprocess = await gdrive_download(
        event, legendevent, service, drive_link
    )
    if legendprocess is not None:
...
async def legendshare(event):
    ...
    await gshare(service, legendevent, input_str)
