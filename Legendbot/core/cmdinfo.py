...
async def grpinfo():
    outstr = "**Plugins in LegendUserBot are:**\n\n"
    outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    for lol in category:
        plugins = GRP_INFO[lol]
        outstr += f"**{hemojis[lol]} {lol.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your LegendUserBot are :**\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    for lol in category:
        plugins = GRP_INFO[lol]
        outstr += f"**{hemojis[lol]} {lol.title()} ** - {len(plugins)}\n\n"
